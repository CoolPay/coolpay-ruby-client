  ...
  module API
    class Request
      ...
      def request method, path, data = {}
        ...
        CoolPay.logger.debug { "#{method.to_s.upcase} #{base_uri}#{path} #{http_options}" }
      ...
      def raise_error body, status
        ...
        klass =
          begin
            require "coolpay/api/errors/#{code}"
            class_name = code.split('_').map(&:capitalize).join('')
            CoolPay::API.const_get(class_name)
      ...
      def headers
        heads = {
          ...
          'Accept-Version' => "v#{CoolPay::API_VERSION}"
        }
      ...
      def user_agent
        user_agent = "coolpay-ruby-client, v#{CoolPay::VERSION}"
