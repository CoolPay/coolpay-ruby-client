...
describe CoolPay::API::Request do
  let(:req_params){{ secret: secret }}
  let(:handler) { CoolPay::API::Request.new(req_params) }

  describe '.new' do
    it 'should set base uri' do
      expect(handler.class.default_options[:base_uri]).to eq(CoolPay::BASE_URI)
    end
  ...
  describe '.request' do
    ...
    it "should send extra options to httparty" do
      request = CoolPay::API::Request.new(secret: ":secret", verify: false)

      allow(CoolPay::API::Request).to receive(:get).and_call_original
      stub_request(:any, //)

      request.request(:get, "/ping")

      expect(CoolPay::API::Request).to have_received(:get) do |_path, options|
    ...
    context "when data contains a file" do
      it "should send a multipart request" do
        request = CoolPay::API::Request.new(secret: "secret")
  ...
  describe '.create_response' do
    ...
    context 'when raw is true' do
      it 'should return raw response', wip: true do
        body = { "id" => 100 }
        stub_request(:get, "https://api.coolpay.net/dummy").to_return(
          :status => 200,
          :body => body.to_json,
    ...
    context 'when content-type is not json' do
      it 'should return raw response' do
        body = 'foobar'
        stub_request(:get, "https://api.coolpay.net/dummy").
  ...
  describe '.raise_error' do
    it 'should raise error' do
      ...
      }.to raise_error(CoolPay::API::BadRequest)

      expect{
        handler.send(:raise_error, "error", 401)
      }.to raise_error(CoolPay::API::Unauthorized)
    ...
    it 'should raise general error if not recognized' do
      ...
      }.to raise_error(CoolPay::API::Error)
    end
  ...
  describe '.headers' do
    ...
    it 'should have no authorization with empty secret' do
      expect(CoolPay::API::Request.new.send(:headers)['Authorization']).to be_nil
    end
  end
end
